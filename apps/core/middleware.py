"""
Language & timezone middleware for Shaks LMS.
...
class LanguageMiddleware:
    """Django WSGI/ASGI middleware that activates language and timezone per request."""
    ...
    def _resolve_language(self, request) -> str:
        # Priority 1: authenticated user profile language
        user = getattr(request, 'user', None)
...
class JWTAuthMiddleware(BaseMiddleware):
    """Channels middleware that authenticates WebSocket connections via JWT query param."""
...
def JWTAuthMiddlewareStack(inner):
    """Convenience wrapper combining JWTAuthMiddleware with the standard auth stack."""
