"""
Async stats view — fetches external APIs concurrently with asyncio.gather.

Why async? This view makes two outbound HTTP calls to external APIs.
If written synchronously each call would block a worker thread for the
duration of the request. Using async + asyncio.gather lets both calls
happen in parallel, so the total latency equals the slower of the two
calls (not the sum of both).
"""
...
async def _fetch_current_time(client: httpx.AsyncClient) -> str:
    """Fetch current Almaty time from timeapi.io."""
    try:
...
def _get_platform_stats() -> dict[str, int]:
    """Read local DB counts. Wrapped in sync_to_async because ORM is synchronous."""
...
class StatsView(APIView):
    ...
    Returns platform statistics combined with live external data.
    Both external API calls run concurrently via asyncio.gather.
